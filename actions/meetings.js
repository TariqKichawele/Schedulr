...
export async function getUserMeetings(type = 'upcoming') {
    ...
    const meetings = await db.booking.findMany({
        where: {
            userId: user.id,
            startTime: type === 'upcoming' ? { gte: now } : { lte: now },
        },
        include: {
            event: {
                include: {
...
export async function cancelMeeting(meetingId) {
    ...
    if(!meeting || meeting.userId !== user.id) throw new Error('Meeting not found or unauthorized');
    ...
    try {
        ...
        console.error('Error canceling meeting in Google Calendar:', error);
    };
